...
def scrapePage(productURL, offset, rootXML, productName, modelNumber, productID, price):
	global j
	# Make TCP Request
	...
	for review in reviews:
		...
		ET.SubElement(reviewTag, "price").text = price		
...
try:
	...
	# Log the event and stack trace within error_log.txt
	error_time = datetime.datetime.now()
	error_string = "------------------------------ Logged: " + error_time + " ------------------------------\n"
...
for product in productInfo:
	...
	while (offsetSt <= numReviews):
		if (scrapePage(product[2], offsetSt, rootXML, product[1], product[3], product[0], product[5])):
...
call("winscp.com /script=ftpUploadLowes.txt")